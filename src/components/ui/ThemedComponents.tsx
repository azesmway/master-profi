...
import {
  ...
  type ViewProps, type TextProps,
  type TextInputProps, type PressableProps,
} from 'react-native';
...
export function ThemedText({ style, ...props }: TextProps) {
  ...
  return (
    <Text
      style={[{ color: colors.text, fontSize: 14 }, style]}
      {...props}
    />
  );
}
...
export function MutedText({ style, ...props }: TextProps) {
  ...
  return (
    <Text
      style={[{ color: colors.textSecondary, fontSize: 13 }, style]}
      {...props}
    />
  );
}
