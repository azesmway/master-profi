...
function ChatRow({ room, myId, onPress, colors }: { room: ChatRoom; myId: string; onPress: () => void; colors: any }) {
    ...
    <Pressable
      ...
      <View style={{ position: 'relative' }}>
        <View
          ...
          <Text style={{ color: '#FF6B35', fontWeight: '700', fontSize: 14 }}>{initials}</Text>
        </View>
        {(room.unreadCount ?? 0) > 0 && (
          <View
            ...
            <Text style={{ color: '#fff', fontSize: 11, fontWeight: '700' }}>{room.unreadCount}</Text>
          </View>
        )}
      </View>
      ...
      <View style={{ flex: 1 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 3 }}>
          ...
          {last && <Text style={{ color: colors.textMuted, fontSize: 12 }}>{timeLabel(last.createdAt)}</Text>}
        </View>
        {last && (
          <Text style={{ fontSize: 14, color: (room.unreadCount ?? 0) > 0 ? colors.text : colors.textSecondary }} numberOfLines={1}>
            {last.senderId === myId ? 'Вы: ' : ''}
            {last.content}
          </Text>
        )}
      </View>
    ...
    <View style={[s.screen, { paddingTop: insets.top }]}>
      <View style={{ paddingHorizontal: 20, paddingTop: 16, paddingBottom: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
        <View>
          ...
          {totalUnread > 0 && <Text style={{ color: '#FF6B35', fontSize: 12, marginTop: 2 }}>{totalUnread} непрочитанных</Text>}
        </View>
      </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={{ alignItems: 'center', paddingVertical: 64, paddingHorizontal: 32 }}>
              ...
              <Text style={[s.textSecondary, { textAlign: 'center' }]}>Чаты появятся когда вы примете отклик специалиста</Text>
            </View>
          }
          renderItem={({ item, index }: any) => (
            <Animated.View entering={FadeInDown.delay(index * 50).springify()}>
              <ChatRow room={item} myId={myId} colors={colors} onPress={() => router.push(`/chat/${item.id}`)} />
            </Animated.View>
          )}
        />
      )}
