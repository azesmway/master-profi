...
const CACHE_VERSION = 'master-v1.2.0';
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(
        keys
          .filter((key) => key.startsWith('master-') && key !== STATIC_CACHE && key !== API_CACHE)
...
self.addEventListener('fetch', (event) => {
  ...
  // API запросы — network first, fallback cache
  if (url.hostname.includes('api.master') || url.pathname.startsWith('/v1/')) {
...
function buildActions(type) {
  switch (type) {
    ...
    case 'response_accepted':
      return [
        { action: 'view', title: '✅ Открыть заказ' },
      ];
    default:
